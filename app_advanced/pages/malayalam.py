...
try:
    ...
    # ============================================
    # LANGUAGE‑SPECIFIC TEXTS (Malayalam)
    # ============================================
    TEXTS = {
        'app_subtitle': "ഇന്ത്യയിലെ ആദ്യത്തെ ബഹുഭാഷാ തട്ടിപ്പ് കണ്ടെത്തൽ സംവിധാനം",
        'tagline': 'നിർഭയ തട്ടിപ്പ് പരിച – 1.4 ബില്യൺ ഇന്ത്യക്കാരെ അവരുടെ ഭാഷയിൽ സംരക്ഷിക്കുന്നു',
        'detector': '🔍 കണ്ടെത്തൽ',
        'learn': '🎓 പഠിക്കുക',
        'help': '🆘 സഹായം',
        'stats': '📊 സ്ഥിതിവിവരക്കണക്കുകൾ',
        'community': '👥 സമൂഹം',
        'about': 'ℹ️ പ്രോജക്റ്റിനെക്കുറിച്ച്',
        'login': '🔐 ലോഗിൻ / ഉപയോക്തൃനാമം',
        'language': '🌐 ഭാഷ',
        'fraud_type': '🎯 തട്ടിപ്പ് തരം',
        'today_stats': "📊 ഇന്നത്തെ സ്ഥിതിവിവരക്കണക്കുകൾ",
        'scans_today': 'ഇന്നത്തെ സ്കാൻ',
        'frauds_blocked': 'തടയപ്പെട്ട തട്ടിപ്പുകൾ',
        ...
        'social': '👥 സോഷ്യൽ മീഡിയ',
        'website': '🌐 വെബ്സൈറ്റ്',
        'enter_sms': '📝 എസ്എംഎസ്/സന്ദേശം നൽകുക',
        'sender_id': '📱 അയച്ചയാളുടെ ഐഡി (ഓപ്ഷണൽ)',
        'high_risk': '🚨 ഉയർന്ന അപകടസാധ്യത കണ്ടെത്തി!',
        'medium_risk': '⚠️ ഇടത്തരം അപകടസാധ്യത',
        'low_risk': '✅ കുറഞ്ഞ അപകടസാധ്യത',
        'risk_score': 'അപകട സ്കോർ',
        'detailed_analysis': '🔍 വിശദമായ വിശകലനം',
        'helplines': '📞 സഹായ നമ്പറുകൾ',
        'share_alert': '📤 അലേർട്ട് പങ്കിടുക',
        'detected_language': '✅ കണ്ടെത്തിയ ഭാഷ',
        'confidence': 'ആത്മവിശ്വാസം',
        'footer_text': '🇮🇳 അഭയം – ഇന്ത്യയെ തട്ടിപ്പിൽ നിന്ന് സംരക്ഷിക്കുന്നു | 12 ഇന്ത്യൻ ഭാഷകൾ | 24x7 സൗജന്യ സേവനം',
        'footer_helpline': '📞 ദേശീയ സൈബർ ക്രൈം സഹായ നമ്പർ: 1930 | 📧 report@cybercrime.gov.in',
        'footer_copyright': '© 2024 അഭയം – എംഎസ്സി ഡാറ്റ സയൻസ് & എഐ പ്രോജക്റ്റ്',
        'sms_placeholder': 'ഏതെങ്കിലും സംശയാസ്പദമായ സന്ദേശം ഇവിടെ ഒട്ടിക്കുക...',
    ...
    with st.sidebar:
        ...
        if not st.session_state.logged_in:
            if st.button("🔐  ലോഗിൻ ചെയ്യുക", use_container_width=True):
                st.query_params["lang"] = "മലയാളം"
                st.query_params["redirect"] = "pages/malayalam.py"
                st.switch_page("pages/login.py")
            st.info("തട്ടിപ്പ് കണ്ടെത്തൽ സവിശേഷതകൾ ആക്സസ് ചെയ്യാൻ ദയവായി ലോഗിൻ ചെയ്യുക")
        else:
            st.success(f"✅ ലോഗിൻ ചെയ്തു: **{st.session_state.username}**")
            if st.button("🚪 ലോഗൗട്ട്", use_container_width=True):
                st.session_state.logged_in = False
                st.session_state.username = ''
                st.rerun()
        st.markdown("---")
        ...
        # Set default selection to current language (Malayalam)
        default_index = list(languages.keys()).index('മലയാളം')
    ...
    with tabs[0]:
        ...
        with col_input:
            ...
            st.markdown("### 📥 ഇൻപുട്ട്", unsafe_allow_html=True)
            
            method = st.radio("ഇൻപുട്ട് രീതി തിരഞ്ഞെടുക്കുക", ["ടെക്സ്റ്റ്", "ചിത്രം", "ഇമെയിൽ"], horizontal=True)
            text_input = ""
            sender_id = ""
            
            if method == "ടെക്സ്റ്റ്":
                text_input = st.text_area(TEXTS['enter_sms'], height=150, placeholder=TEXTS['sms_placeholder'])
                sender_id = st.text_input(TEXTS['sender_id'], placeholder="ഉദാ, BANKALERT")
            elif method == "ചിത്രം":
                st.markdown('<div style="border:2px dashed #18B7BE; border-radius:20px; padding:20px;">', unsafe_allow_html=True)
                uploaded = st.file_uploader("ചിത്രം അപ്‌ലോഡ് ചെയ്യുക", type=['png','jpg','jpeg'], label_visibility="collapsed")
                st.markdown('</div>', unsafe_allow_html=True)
                if uploaded:
                    with st.spinner("ടെക്സ്റ്റ് എക്സ്ട്രാക്റ്റ് ചെയ്യുന്നു..."):
                        progress_bar = st.progress(0)
                        for i in range(100):
                            time.sleep(0.01)
                            progress_bar.progress(i + 1)
                        text_input = extract_text_from_image(uploaded)
                        st.text_area("എക്സ്ട്രാക്റ്റ് ചെയ്ത ടെക്സ്റ്റ്", text_input, height=150)
            else:  # ഇമെയിൽ
                text_input = st.text_area("ഇമെയിലിന്റെ ഉള്ളടക്കം", height=150)
                sender_id = st.text_input("അയച്ചയാളുടെ ഇമെയിൽ", placeholder="ayachavan@example.com")
            
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("🔍 ഭാഷ കണ്ടെത്തുക", use_container_width=True):
                    if text_input:
                        with st.spinner("ഭാഷ കണ്ടെത്തുന്നു..."):
                            progress_bar = st.progress(0)
                            for i in range(100):
                                time.sleep(0.01)
                                progress_bar.progress(i + 1)
            ...
            with col_btn2:
                if st.button("🚨 തട്ടിപ്പ് പരിശോധിക്കുക", type="primary", use_container_width=True):
                    if not text_input:
                        st.warning("ദയവായി കുറച്ച് വാചകം നൽകുക.")
                    else:
                        with st.spinner("വിശകലനം ചെയ്യുന്നു..."):
                            ...
                            try:
                                ...
                                ml_prob = 0.5
                                model_source = "റൂൾ എഞ്ചിൻ മാത്രം"
        ...
        with col_output:
            if st.session_state.results:
                ...
                if companies:
                    st.markdown("### 🏢 കമ്പനി പരിശോധന")
                    for comp in companies:
                        st.markdown(f"**{comp['name']}**")
                        st.caption(f"ഔദ്യോഗിക ഡൊമെയ്ൻ: {comp['domain']}")
                        if res['sender']:
                            if verify_sender(comp['domain'], res['sender']):
                                st.success("✅ അയച്ചയാളുടെ ഡൊമെയ്ൻ ഒത്തുപോകുന്നു")
                            else:
                                st.warning("⚠️ അയച്ചയാളുടെ ഡൊമെയ്ൻ ഒത്തുപോകുന്നില്ല")
                ...
                if st.session_state.username:
                    st.markdown(f"### {TEXTS['share_alert']} / ഫീഡ്ബാക്ക്")
                    ...
                    with col_f3:
                        if st.button("🔗 തെളിവ് ലിങ്ക്"):
                            st.session_state.show_proof = True
                    
                    if st.session_state.show_feedback:
                        comment = st.text_area("എന്താണ് തെറ്റ്?")
                        if st.button("ഫീഡ്ബാക്ക് സമർപ്പിക്കുക"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, comment, "")
                            st.success("ഫീഡ്ബാക്ക് രേഖപ്പെടുത്തി")
                            st.session_state.show_feedback = False
                    
                    if st.session_state.show_proof:
                        proof = st.text_input("ഔദ്യോഗിക ലിങ്ക് ഒട്ടിക്കുക")
                        if st.button("തെളിവ് സമർപ്പിക്കുക"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, "", proof)
                            st.success("തെളിവ് സമർപ്പിച്ചു")
                            st.session_state.show_proof = False
                else:
                    st.info("ഫീഡ്ബാക്ക് നൽകാൻ ദയവായി ലോഗിൻ ചെയ്യുക.")
                
                share_text = f"ഞാൻ ഈ സന്ദേശം അഭയം ഉപയോഗിച്ച് പരിശോധിച്ചു. അപകടസാധ്യത: {result['final_score']}% ({risk_level}). നിങ്ങളും പരിശോധിക്കൂ!"
                col_s1, col_s2, col_s3 = st.columns(3)
                with col_s1:
                    st.markdown(f"[![WhatsApp](https://img.icons8.com/color/48/000000/whatsapp--v1.png)]({get_whatsapp_link(share_text)})")
                with col_s2:
                    st.markdown(f"[![Email](https://img.icons8.com/color/48/000000/email.png)]({get_email_link('തട്ടിപ്പ് അലേർട്ട്', share_text)})")
                with col_s3:
    ...
    with tabs[1]:
        ...
        st.write("തട്ടിപ്പുകളെക്കുറിച്ചുള്ള വിദ്യാഭ്യാസ ഉള്ളടക്കം ഇവിടെ ചേർക്കാം – ലേഖനങ്ങൾ, വീഡിയോകൾ മുതലായവ.")
    ...
    with tabs[3]:
        ...
        with col_filter1:
            date_range = st.date_input("തീയതി പരിധി തിരഞ്ഞെടുക്കുക", [dates[0], dates[-1]])
        with col_filter2:
            fraud_types = st.multiselect("തട്ടിപ്പ് തരം", df['Type'].unique(), default=df['Type'].unique())

        risk_threshold = st.slider("അപകട പരിധി (%)", 0, 100, 50)

        filtered_df = df[(df['Date'] >= pd.Timestamp(date_range[0])) & 
                         (df['Date'] <= pd.Timestamp(date_range[1])) &
                         (df['Type'].isin(fraud_types))]

        fig = px.line(filtered_df, x='Date', y='Fraud Cases', title='ദൈനംദിന തട്ടിപ്പ് കേസുകൾ',
        ...
        fig2 = px.bar(type_counts, x='Type', y='Fraud Cases', title='തരം അനുസരിച്ച് ആകെ തട്ടിപ്പ്',
    ...
    with tabs[4]:
            ...
            for _, row in posts.sort_values('timestamp', ascending=False).iterrows():
                ...
                <div style="background:rgba(10,12,16,0.6); border-radius:24px; padding:1.5rem; margin-bottom:1rem; border:1px solid #2D3A46;">
                    ...
                    <div style="margin-top:0.8rem;">
                        <span style="color:#18B7BE;">വിഭാഗം: {row['category']}</span>
                        {f' <a href="{row["link"]}" target="_blank" style="color:#18B7BE;">🔗 ലിങ്ക്</a>' if row['link'] else ''}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("ഇതുവരെ പോസ്റ്റുകളൊന്നുമില്ല. ആദ്യ പോസ്റ്റ് സൃഷ്ടിക്കുക!")
        ...
        if st.session_state.username:
            ...
            with st.form("new_post_form"):
                ...
                category = st.selectbox(TEXTS['category'], ["പൊതുവായ", "തട്ടിപ്പ് അലേർട്ട്", "ചോദ്യം", "വിജയഗാഥ"])
                ...
                if submitted and title and content:
                    ...
                    st.success("പോസ്റ്റ് ചേർത്തു!")
                    st.rerun()
        else:
            st.info("പോസ്റ്റ് ചെയ്യാൻ ദയവായി ലോഗിൻ ചെയ്യുക.")
    ...
    with tabs[5]:
        ...
        <div style="background:rgba(10,12,16,0.6); border-radius:32px; padding:2rem; border:1px solid #2D3A46;">
            <h3 style="color:#FEDD89;">അഭയം – നിർഭയ തട്ടിപ്പ് പരിച</h3>
            <p style="color:#EFF7F6;"><strong>എന്തുകൊണ്ട് അഭയം?</strong> ലക്ഷക്കണക്കിന് ആളുകൾ അവരുടെ മാതൃഭാഷയിൽ തട്ടിപ്പുകൾക്ക് ഇരയാകുന്ന ഒരു രാജ്യത്ത്, ഞങ്ങൾ നിങ്ങളെ മനസ്സിലാക്കുന്ന ഒരു സംവിധാനം നിർമ്മിച്ചു – നിങ്ങൾ ഹിന്ദി, മറാത്തി, തമിഴ് അല്ലെങ്കിൽ ഇന്ത്യയിലെ 12 ഔദ്യോഗിക ഭാഷകളിൽ ഏത് സംസാരിച്ചാലും. നൂതന മെഷീൻ ലേണിംഗും ശക്തമായ റൂൾ എഞ്ചിനും സംയോജിപ്പിച്ച്, അഭയം 1.4 ബില്യൺ ഇന്ത്യക്കാരെ തട്ടിപ്പിൽ നിന്ന് സംരക്ഷിക്കുന്നു, ഒരു സന്ദേശത്തിൽ.</p>
            <hr style="border-color:#2D3A46;">
            <p><strong style="color:#18B7BE;">👩‍💻 സ്രഷ്ടാവ്:</strong> <span style="color:#9BB8C9;">സാക്ഷി ശ്രീകൃഷ്ണ മഗം</span></p>
            <p><strong style="color:#18B7BE;">🎓 പഠിക്കുന്നത്:</strong> <span style="color:#9BB8C9;">എംഎസ്സി ഡാറ്റ സയൻസ് ആൻഡ് ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ്</span></p>
            <p><strong style="color:#18B7BE;">🏛️ സ്ഥാപനം:</strong> <span style="color:#9BB8C9;">രാംനിരഞ്ജൻ ഝുൻഝുൻവാല കോളേജ്</span></p>
            <p><strong style="color:#18B7BE;">🔧 സാങ്കേതികവിദ്യകൾ:</strong> <span style="color:#9BB8C9;">Python, Scikit‑learn, XGBoost, Streamlit, കസ്റ്റം റൂൾ എഞ്ചിൻ, ബഹുഭാഷാ NLP, OCR, എന്നിവയും മറ്റും.</span></p>
            <p><strong style="color:#18B7BE;">📧 ബന്ധപ്പെടുക:</strong> <span style="color:#9BB8C9;">sakshimagham@gmail.com</span></p>
            <p><strong style="color:#18B7BE;">🌟 ദർശനം:</strong> <span style="color:#9BB8C9;">ഓരോ പൗരനും തത്സമയ, ഭാഷ-സ്വതന്ത്ര സംരക്ഷണം നൽകി തട്ടിപ്പ് രഹിത ഡിജിറ്റൽ ഇന്ത്യ സൃഷ്ടിക്കുക.</span></p>
            <p><em style="color:#DD8EA4;">എംഎസ്സി ഡാറ്റ സയൻസ് & എഐ കോഴ്സിന്റെ അവസാന വർഷ ആവശ്യകതയായി ഈ പ്രോജക്റ്റ് സമർപ്പിക്കുന്നു. ഇത് ഒരു മാറ്റമുണ്ടാക്കുമെന്ന് ഞങ്ങൾ പ്രതീക്ഷിക്കുന്നു.</em></p>
        </div>
        """, unsafe_allow_html=True)
