...
try:
    ...
    # ============================================
    # LANGUAGE‑SPECIFIC TEXTS (Marathi)
    # ============================================
    TEXTS = {
        'app_subtitle': "भारताची पहिली बहुभाषिक फसवणूक तपास प्रणाली",
        'tagline': 'निर्भय फसवणूक ढाल – १.४ अब्ज भारतीयांना त्यांच्या भाषेत संरक्षण',
        'detector': '🔍 तपासक',
        'learn': '🎓 शिका',
        'help': '🆘 मदत',
        'stats': '📊 आकडेवारी',
        'community': '👥 समुदाय',
        'about': 'ℹ️ माहिती',
        'login': '🔐 लॉगिन / वापरकर्तानाव',
        'language': '🌐 भाषा',
        'fraud_type': '🎯 फसवणूक प्रकार',
        'today_stats': "📊 आजची आकडेवारी",
        'scans_today': 'आजच्या तपासण्या',
        'frauds_blocked': 'फसवणूक रोखल्या',
        ...
        'social': '👥 सोशल मीडिया',
        'website': '🌐 वेबसाइट',
        'enter_sms': '📝 एसएमएस/संदेश लिहा',
        'sender_id': '📱 पाठवणारा आयडी (वैकल्पिक)',
        'high_risk': '🚨 उच्च धोका आढळला!',
        'medium_risk': '⚠️ मध्यम धोका',
        'low_risk': '✅ कमी धोका',
        'risk_score': 'धोका स्कोअर',
        'detailed_analysis': '🔍 तपशीलवार विश्लेषण',
        'helplines': '📞 हेल्पलाइन',
        'share_alert': '📤 अलर्ट शेअर करा',
        'detected_language': '✅ आढळलेली भाषा',
        'confidence': 'विश्वास',
        'footer_text': '🇮🇳 अभयम् – भारताला फसवणुकीपासून वाचवा | १२ भारतीय भाषा | २४x७ मोफत सेवा',
        'footer_helpline': '📞 राष्ट्रीय सायबर क्राइम हेल्पलाइन: 1930 | 📧 report@cybercrime.gov.in',
        'footer_copyright': '© २०२४ अभयम् – एमएससी डेटा सायन्स आणि एआय प्रकल्प',
        'sms_placeholder': 'कोणताही संशयास्पद संदेश येथे पेस्ट करा...',
        'new_post': '📝 नवीन पोस्ट',
        'author': 'लेखक',
        'title': 'शीर्षक',
        'content': 'सामग्री',
    ...
    with st.sidebar:
        ...
        if not st.session_state.logged_in:
            if st.button("🔐  लॉगिन करा", use_container_width=True):
                st.query_params["lang"] = "मराठी"
                st.query_params["redirect"] = "pages/marathi.py"
                st.switch_page("pages/login.py")
            st.info("फसवणूक शोध वैशिष्ट्यांमध्ये प्रवेश करण्यासाठी कृपया लॉगिन करा")
        else:
            st.success(f"✅  लॉगिन केले: **{st.session_state.username}**")
        ...
        # Set default selection to current language (Marathi)
        default_index = list(languages.keys()).index('मराठी')
        selected_lang = st.selectbox("Select Language", list(languages.keys()), index=default_index)
        if selected_lang != "मराठी":
    ...
    with tabs[0]:
        ...
        with col_input:
            ...
            method = st.radio("इनपुट पद्धत निवडा", ["मजकूर", "प्रतिमा", "ईमेल"], horizontal=True)
            text_input = ""
            sender_id = ""
            
            if method == "मजकूर":
                text_input = st.text_area(TEXTS['enter_sms'], height=150, placeholder=TEXTS['sms_placeholder'])
                sender_id = st.text_input(TEXTS['sender_id'], placeholder="उदा., BANKALERT")
            elif method == "प्रतिमा":
                st.markdown('<div style="border:2px dashed #18B7BE; border-radius:20px; padding:20px;">', unsafe_allow_html=True)
                uploaded = st.file_uploader("प्रतिमा अपलोड करा", type=['png','jpg','jpeg'], label_visibility="collapsed")
                st.markdown('</div>', unsafe_allow_html=True)
                if uploaded:
                    with st.spinner("मजकूर काढला जात आहे..."):
                        progress_bar = st.progress(0)
                        for i in range(100):
                            time.sleep(0.01)
                            progress_bar.progress(i + 1)
                        text_input = extract_text_from_image(uploaded)
                        st.text_area("काढलेला मजकूर", text_input, height=150)
            else:  # ईमेल
                text_input = st.text_area("ईमेलचा मजकूर", height=150)
                sender_id = st.text_input("प्रेषक ईमेल", placeholder="prerak@example.com")
            
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("🔍 भाषा ओळखा", use_container_width=True):
                    if text_input:
                        with st.spinner("भाषा ओळखत आहे..."):
                            progress_bar = st.progress(0)
                            for i in range(100):
                                time.sleep(0.01)
                                progress_bar.progress(i + 1)
            ...
            with col_btn2:
                if st.button("🚨 फसवणूक तपासा", type="primary", use_container_width=True):
                    if not text_input:
                        st.warning("कृपया काही मजकूर प्रविष्ट करा.")
                    else:
                        with st.spinner("विश्लेषण करत आहे..."):
                            progress_bar = st.progress(0)
                            for i in range(100):
                                time.sleep(0.02)
                                progress_bar.progress(i + 1)
        ...
        with col_output:
            if st.session_state.results:
                ...
                if companies:
                    st.markdown("### 🏢 कंपनी पडताळणी")
                    for comp in companies:
                        st.markdown(f"**{comp['name']}**")
                        st.caption(f"अधिकृत डोमेन: {comp['domain']}")
                        if res['sender']:
                            if verify_sender(comp['domain'], res['sender']):
                                st.success("✅ प्रेषक डोमेन जुळले")
                            else:
                                st.warning("⚠️ प्रेषक डोमेन जुळले नाही")
                ...
                if st.session_state.username:
                    st.markdown(f"### {TEXTS['share_alert']} / अभिप्राय")
                    ...
                    with col_f3:
                        if st.button("🔗 पुरावा लिंक"):
                            st.session_state.show_proof = True
                    
                    if st.session_state.show_feedback:
                        comment = st.text_area("काय चूक होती?")
                        if st.button("अभिप्राय पाठवा"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, comment, "")
                            st.success("अभिप्राय नोंदवला")
                            st.session_state.show_feedback = False
                    
                    if st.session_state.show_proof:
                        proof = st.text_input("अधिकृत लिंक पेस्ट करा")
                        if st.button("पुरावा पाठवा"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, "", proof)
                            st.success("पुरावा सादर केला")
                            st.session_state.show_proof = False
                else:
                    st.info("अभिप्राय देण्यासाठी कृपया लॉगिन करा.")
                
                share_text = f"मी हा संदेश अभयम् सोबत तपासला. धोका: {result['final_score']}% ({risk_level}). तुम्ही स्वतः तपासा!"
                col_s1, col_s2, col_s3 = st.columns(3)
                with col_s1:
                    st.markdown(f"[![WhatsApp](https://img.icons8.com/color/48/000000/whatsapp--v1.png)]({get_whatsapp_link(share_text)})")
                with col_s2:
                    st.markdown(f"[![Email](https://img.icons8.com/color/48/000000/email.png)]({get_email_link('फसवणूक अलर्ट', share_text)})")
                with col_s3:
    ...
    with tabs[1]:
        ...
        st.write("तुम्ही येथे फसवणूकीबद्दल माहिती जोडू शकता – लेख, व्हिडिओ इ.")
    ...
    with tabs[3]:
        ...
        with col_filter1:
            date_range = st.date_input("तारीख श्रेणी निवडा", [dates[0], dates[-1]])
        with col_filter2:
            fraud_types = st.multiselect("फसवणूक प्रकार", df['Type'].unique(), default=df['Type'].unique())

        risk_threshold = st.slider("धोका मर्यादा (%)", 0, 100, 50)

        filtered_df = df[(df['Date'] >= pd.Timestamp(date_range[0])) & 
                         (df['Date'] <= pd.Timestamp(date_range[1])) &
                         (df['Type'].isin(fraud_types))]

        fig = px.line(filtered_df, x='Date', y='Fraud Cases', title='दैनिक फसवणूक प्रकरणे',
        ...
        fig2 = px.bar(type_counts, x='Type', y='Fraud Cases', title='प्रकारानुसार एकूण फसवणूक',
    ...
    with tabs[4]:
        ...
        if not posts.empty:
            ...
            st.info("अद्याप कोणतीही पोस्ट नाही. पहिली पोस्ट करा!")
        ...
        if st.session_state.username:
            ...
            with st.form("new_post_form"):
                ...
                category = st.selectbox(TEXTS['category'], ["सामान्य", "फसवणूक अलर्ट", "प्रश्न", "यशोगाथा"])
                ...
                if submitted and title and content:
                    ...
                    st.success("पोस्ट जोडली!")
                    st.rerun()
        else:
            st.info("पोस्ट तयार करण्यासाठी कृपया लॉगिन करा.")
    ...
    with tabs[5]:
        ...
        <div style="background:rgba(10,12,16,0.6); border-radius:32px; padding:2rem; border:1px solid #2D3A46;">
            <h3 style="color:#FEDD89;">अभयम् – निर्भय फसवणूक ढाल</h3>
            <p style="color:#EFF7F6;"><strong>अभयम् का?</strong> अशा देशात जिथे लाखो लोक त्यांच्या मातृभाषेत फसवणुकीला बळी पडतात, आम्ही एक अशी प्रणाली बनवली आहे जी तुम्हाला समजते – मग तुम्ही हिंदी, मराठी, तामिळ किंवा भारतातील इतर १२ अधिकृत भाषांपैकी कोणतीही भाषा बोलत असलात तरी. प्रगत मशीन लर्निंग आणि एक शक्तिशाली नियम इंजिन यांच्या संयोगाने, अभयम् १.४ अब्ज भारतीयांना फसवणुकीपासून संरक्षण देते, एका संदेशावेळी.</p>
            ...
            <p><strong style="color:#18B7BE;">🎓 शिक्षण:</strong> <span style="color:#9BB8C9;">एमएससी डेटा सायन्स आणि आर्टिफिशियल इंटेलिजन्स</span></p>
            <p><strong style="color:#18B7BE;">🏛️ संस्था:</strong> <span style="color:#9BB8C9;">रामनिरंजन झुनझुनवाला महाविद्यालय</span></p>
            <p><strong style="color:#18B7BE;">🔧 तंत्रज्ञान:</strong> <span style="color:#9BB8C9;">Python, Scikit‑learn, XGBoost, Streamlit, कस्टम नियम इंजिन, बहुभाषिक NLP, OCR, आणि अधिक.</span></p>
            <p><strong style="color:#18B7BE;">📧 संपर्क:</strong> <span style="color:#9BB8C9;">sakshimagham@gmail.com</span></p>
            <p><strong style="color:#18B7BE;">🌟 ध्येय:</strong> <span style="color:#9BB8C9;">प्रत्येक नागरिकाला रिअल‑टाइम, भाषा-अज्ञेय संरक्षण देऊन एक फसवणूक-मुक्त डिजिटल भारत निर्माण करणे.</span></p>
            <p><em style="color:#DD8EA4;">हा प्रकल्प एमएससी डेटा सायन्स आणि एआय अभ्यासक्रमाच्या अंतिम वर्षाच्या आवश्यकतेसाठी सादर केला आहे. आम्हाला आशा आहे की यामुळे फरक पडेल.</em></p>
        </div>
        """, unsafe_allow_html=True)
