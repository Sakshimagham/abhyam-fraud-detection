...
try:
    ...
    # ============================================
    # LANGUAGE‑SPECIFIC TEXTS (Hindi)
    # ============================================
    TEXTS = {
        'app_subtitle': "भारत की पहली बहुभाषी धोखाधड़ी जांच प्रणाली",
        'tagline': 'निडर धोखाधड़ी ढाल – १.४ अरब भारतीयों को उनकी भाषा में सुरक्षा',
        'detector': '🔍 जांचकर्ता',
        'learn': '🎓 सीखें',
        'help': '🆘 सहायता',
        'stats': '📊 आंकड़े',
        'community': '👥 समुदाय',
        'about': 'ℹ️ परियोजना के बारे में',
        'login': '🔐 लॉगिन / उपयोगकर्ता नाम',
        'language': '🌐 भाषा',
        'fraud_type': '🎯 धोखाधड़ी प्रकार',
        'today_stats': "📊 आज के आंकड़े",
        'scans_today': 'आज की जांच',
        'frauds_blocked': 'धोखाधड़ी रोके',
        ...
        'social': '👥 सोशल मीडिया',
        'website': '🌐 वेबसाइट',
        'enter_sms': '📝 एसएमएस/संदेश लिखें',
        'sender_id': '📱 भेजने वाला आईडी (वैकल्पिक)',
        'high_risk': '🚨 उच्च जोखिम पाया गया!',
        'medium_risk': '⚠️ मध्यम जोखिम',
        'low_risk': '✅ कम जोखिम',
        'risk_score': 'जोखिम स्कोर',
        'detailed_analysis': '🔍 विस्तृत विश्लेषण',
        'helplines': '📞 हेल्पलाइन',
        'share_alert': '📤 अलर्ट साझा करें',
        'detected_language': '✅ पहचानी गई भाषा',
        'confidence': 'विश्वास',
        'footer_text': '🇮🇳 अभयम् – भारत को धोखाधड़ी से बचाएं | १२ भारतीय भाषाएं | २४x७ मुफ्त सेवा',
        'footer_helpline': '📞 राष्ट्रीय साइबर क्राइम हेल्पलाइन: 1930 | 📧 report@cybercrime.gov.in',
        'footer_copyright': '© २०२४ अभयम् – एमएससी डेटा साइंस एवं एआई परियोजना',
        'sms_placeholder': 'कोई भी संदिग्ध संदेश यहाँ डालें...',
        'new_post': '📝 नई पोस्ट',
        'author': 'लेखक',
        'title': 'शीर्षक',
        'content': 'सामग्री',
    ...
    with st.sidebar:
        ...
        if not st.session_state.logged_in:
            if st.button("🔐 लॉगिन करें", use_container_width=True):
                st.query_params["lang"] = "हिंदी"
                st.query_params["redirect"] = "pages/hindi.py"
                st.switch_page("pages/login.py")
            st.info("धोखाधड़ी पहचान सुविधाओं तक पहुंचने के लिए कृपया लॉगिन करें")
        else:
            st.success(f"✅ लॉगिन किया: **{st.session_state.username}**")
        ...
        # Set default selection to current language (Hindi)
        default_index = list(languages.keys()).index('हिंदी')
    ...
    <div class="title-wrapper">
        ...
        <div class="lang-letter lang3">ब</div>
        <div class="lang-letter lang4">त</div>
    ...
    with tabs[0]:
        ...
        with col_input:
            ...
            st.markdown("### 📥 इनपुट", unsafe_allow_html=True)
            
            method = st.radio("इनपुट विधि चुनें", ["पाठ", "छवि", "ईमेल"], horizontal=True)
            text_input = ""
            sender_id = ""
            
            if method == "पाठ":
                text_input = st.text_area(TEXTS['enter_sms'], height=150, placeholder=TEXTS['sms_placeholder'])
                sender_id = st.text_input(TEXTS['sender_id'], placeholder="जैसे, BANKALERT")
            elif method == "छवि":
                st.markdown('<div style="border:2px dashed #18B7BE; border-radius:20px; padding:20px;">', unsafe_allow_html=True)
                uploaded = st.file_uploader("छवि अपलोड करें", type=['png','jpg','jpeg'], label_visibility="collapsed")
                st.markdown('</div>', unsafe_allow_html=True)
                if uploaded:
                    with st.spinner("पाठ निकाला जा रहा है..."):
                        progress_bar = st.progress(0)
                        for i in range(100):
                            time.sleep(0.01)
                            progress_bar.progress(i + 1)
                        text_input = extract_text_from_image(uploaded)
                        st.text_area("निकाला गया पाठ", text_input, height=150)
            else:  # ईमेल
                text_input = st.text_area("ईमेल का पाठ", height=150)
                sender_id = st.text_input("प्रेषक ईमेल", placeholder="bhejne.wala@example.com")
            
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("🔍 भाषा पहचानें", use_container_width=True):
                    if text_input:
                        with st.spinner("भाषा पहचानी जा रही है..."):
                            progress_bar = st.progress(0)
                            for i in range(100):
                                time.sleep(0.01)
                                progress_bar.progress(i + 1)
            ...
            with col_btn2:
                if st.button("🚨 धोखाधड़ी जांचें", type="primary", use_container_width=True):
                    if not text_input:
        ...
        with col_output:
            if st.session_state.results:
                ...
                if companies:
                    st.markdown("### 🏢 कंपनी सत्यापन")
                    for comp in companies:
                        st.markdown(f"**{comp['name']}**")
                        st.caption(f"आधिकारिक डोमेन: {comp['domain']}")
                        if res['sender']:
                            if verify_sender(comp['domain'], res['sender']):
                                st.success("✅ प्रेषक डोमेन मेल खाता है")
                            else:
                                st.warning("⚠️ प्रेषक डोमेन मेल नहीं खाता")
                ...
                if st.session_state.username:
                    st.markdown(f"### {TEXTS['share_alert']} / प्रतिक्रिया")
                    ...
                    with col_f3:
                        if st.button("🔗 प्रमाण लिंक"):
                            st.session_state.show_proof = True
                    
                    if st.session_state.show_feedback:
                        comment = st.text_area("क्या गलत था?")
                        if st.button("प्रतिक्रिया भेजें"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, comment, "")
                            st.success("प्रतिक्रिया दर्ज की गई")
                            st.session_state.show_feedback = False
                    
                    if st.session_state.show_proof:
                        proof = st.text_input("आधिकारिक लिंक पेस्ट करें")
                        if st.button("प्रमाण भेजें"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, "", proof)
                            st.success("प्रमाण प्रस्तुत किया गया")
                            st.session_state.show_proof = False
                else:
                    st.info("प्रतिक्रिया देने के लिए कृपया लॉगिन करें.")
                
                share_text = f"मैंने इस संदेश को अभयम् से जांचा। जोखिम: {result['final_score']}% ({risk_level}). आप भी जांचें!"
                col_s1, col_s2, col_s3 = st.columns(3)
                with col_s1:
                    st.markdown(f"[![WhatsApp](https://img.icons8.com/color/48/000000/whatsapp--v1.png)]({get_whatsapp_link(share_text)})")
                with col_s2:
                    st.markdown(f"[![Email](https://img.icons8.com/color/48/000000/email.png)]({get_email_link('धोखाधड़ी चेतावनी', share_text)})")
                with col_s3:
    ...
    with tabs[1]:
        ...
        st.write("आप यहाँ धोखाधड़ी के बारे में जानकारी जोड़ सकते हैं – लेख, वीडियो आदि।")
    ...
    with tabs[3]:
        ...
        with col_filter1:
            date_range = st.date_input("तारीख सीमा चुनें", [dates[0], dates[-1]])
        with col_filter2:
            fraud_types = st.multiselect("धोखाधड़ी प्रकार", df['Type'].unique(), default=df['Type'].unique())

        risk_threshold = st.slider("जोखिम सीमा (%)", 0, 100, 50)

        filtered_df = df[(df['Date'] >= pd.Timestamp(date_range[0])) & 
                         (df['Date'] <= pd.Timestamp(date_range[1])) &
                         (df['Type'].isin(fraud_types))]

        fig = px.line(filtered_df, x='Date', y='Fraud Cases', title='दैनिक धोखाधड़ी घटनाएं',
        ...
        fig2 = px.bar(type_counts, x='Type', y='Fraud Cases', title='प्रकार के अनुसार कुल धोखाधड़ी',
    ...
    with tabs[4]:
            ...
            for _, row in posts.sort_values('timestamp', ascending=False).iterrows():
                ...
                <div style="background:rgba(10,12,16,0.6); border-radius:24px; padding:1.5rem; margin-bottom:1rem; border:1px solid #2D3A46;">
                    ...
                    <div style="margin-top:0.8rem;">
                        <span style="color:#18B7BE;">श्रेणी: {row['category']}</span>
                        {f' <a href="{row["link"]}" target="_blank" style="color:#18B7BE;">🔗 लिंक</a>' if row['link'] else ''}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("अभी तक कोई पोस्ट नहीं। पहली पोस्ट बनाएं!")
        ...
        if st.session_state.username:
            ...
            with st.form("new_post_form"):
                ...
                category = st.selectbox(TEXTS['category'], ["सामान्य", "धोखाधड़ी चेतावनी", "प्रश्न", "सफलता की कहानी"])
                ...
                if submitted and title and content:
                    ...
                    st.success("पोस्ट जोड़ दी गई!")
                    st.rerun()
        else:
            st.info("पोस्ट बनाने के लिए कृपया लॉगिन करें।")
    ...
    with tabs[5]:
        ...
        <div style="background:rgba(10,12,16,0.6); border-radius:32px; padding:2rem; border:1px solid #2D3A46;">
            <h3 style="color:#FEDD89;">अभयम् – निडर धोखाधड़ी ढाल</h3>
            <p style="color:#EFF7F6;"><strong>अभयम् क्यों?</strong> ऐसे देश में जहाँ लाखों लोग अपनी मातृभाषा में धोखाधड़ी का शिकार होते हैं, हमने एक ऐसी प्रणाली बनाई है जो आपको समझती है – चाहे आप हिंदी, मराठी, तमिल या भारत की 12 आधिकारिक भाषाओं में से कोई भी बोलते हों। उन्नत मशीन लर्निंग और एक शक्तिशाली नियम इंजन के संयोजन से, अभयम् 1.4 अरब भारतीयों को धोखाधड़ी से सुरक्षा प्रदान करता है, एक संदेश पर।</p>
            <hr style="border-color:#2D3A46;">
            <p><strong style="color:#18B7BE;">👩‍💻 निर्माता:</strong> <span style="color:#9BB8C9;">साक्षी श्रीकृष्ण मगम</span></p>
            <p><strong style="color:#18B7BE;">🎓 शिक्षा:</strong> <span style="color:#9BB8C9;">एमएससी डेटा साइंस और आर्टिफिशियल इंटेलिजेंस</span></p>
            <p><strong style="color:#18B7BE;">🏛️ संस्थान:</strong> <span style="color:#9BB8C9;">रामनिरंजन झुनझुनवाला महाविद्यालय</span></p>
            <p><strong style="color:#18B7BE;">🔧 प्रौद्योगिकी:</strong> <span style="color:#9BB8C9;">Python, Scikit‑learn, XGBoost, Streamlit, कस्टम नियम इंजिन, बहुभाषी NLP, OCR, और अधिक।</span></p>
            <p><strong style="color:#18B7BE;">📧 संपर्क:</strong> <span style="color:#9BB8C9;">sakshimagham@gmail.com</span></p>
            <p><strong style="color:#18B7BE;">🌟 लक्ष्य:</strong> <span style="color:#9BB8C9;">प्रत्येक नागरिक को रीयल‑टाइम, भाषा-अज्ञेय सुरक्षा प्रदान करके एक धोखाधड़ी-मुक्त डिजिटल भारत का निर्माण करना।</span></p>
            <p><em style="color:#DD8EA4;">यह परियोजना एमएससी डेटा साइंस और एआई पाठ्यक्रम की अंतिम वर्ष की आवश्यकता के रूप में प्रस्तुत की गई है। हमें आशा है कि यह एक बदलाव लाएगी।</em></p>
        </div>
        """, unsafe_allow_html=True)
