...
try:
    ...
    <style>
        ...
        * {
        ...
        /* Dark background for body (outer edges) – optional, but kept for consistency */
        body {
            background: #0A0C10;
    ...
    # ============================================
    # LANGUAGE‑SPECIFIC TEXTS (Telugu)
    # ============================================
    TEXTS = {
        'app_subtitle': "భారతదేశపు మొట్టమొదటి బహుభాషా మోస గుర్తింపు వ్యవస్థ",
        'tagline': 'నిర్భయ మోసపు కవచం – 1.4 బిలియన్ భారతీయులను వారి స్వంత భాషలో రక్షించడం',
        'detector': '🔍 గుర్తింపు',
        'learn': '🎓 నేర్చుకోండి',
        'help': '🆘 సహాయం',
        'stats': '📊 గణాంకాలు',
        'community': '👥 సమాజం',
        'about': 'ℹ️ ప్రాజెక్ట్ గురించి',
        'login': '🔐 లాగిన్ / వినియోగదారు పేరు',
        'language': '🌐 భాష',
        'fraud_type': '🎯 మోసం రకం',
        'today_stats': "📊 ఈ రోజు గణాంకాలు",
        'scans_today': 'ఈ రోజు స్కాన్లు',
        'frauds_blocked': 'నిరోధించబడిన మోసాలు',
        ...
        'social': '👥 సోషల్ మీడియా',
        'website': '🌐 వెబ్‌సైట్',
        'enter_sms': '📝 SMS/సందేశాన్ని నమోదు చేయండి',
        'sender_id': '📱 పంపినవారి ID (ఐచ్ఛికం)',
        'high_risk': '🚨 అధిక ప్రమాదం కనుగొనబడింది!',
        'medium_risk': '⚠️ మధ్యస్థ ప్రమాదం',
        'low_risk': '✅ తక్కువ ప్రమాదం',
        'risk_score': 'ప్రమాద స్కోరు',
        'detailed_analysis': '🔍 వివరణాత్మక విశ్లేషణ',
        'helplines': '📞 హెల్ప్‌లైన్‌లు',
        'share_alert': '📤 అలర్ట్‌ను భాగస్వామ్యం చేయండి',
        'detected_language': '✅ గుర్తించబడిన భాష',
        'confidence': 'విశ్వాసం',
        'footer_text': '🇮🇳 అభయం – భారతదేశాన్ని మోసాల నుండి రక్షిస్తుంది | 12 భారతీయ భాషలు | 24x7 ఉచిత సేవ',
        'footer_helpline': '📞 జాతీయ సైబర్ క్రైమ్ హెల్ప్‌లైన్: 1930 | 📧 report@cybercrime.gov.in',
        'footer_copyright': '© 2024 అభయం – MSc డేటా సైన్స్ & AI ప్రాజెక్ట్',
        'sms_placeholder': 'ఏదైనా అనుమానాస్పద సందేశాన్ని ఇక్కడ పేస్ట్ చేయండి...',
    ...
    with st.sidebar:
        ...
        if not st.session_state.logged_in:
            if st.button("🔐 లాగిన్ అవ్వండి", use_container_width=True):
                st.query_params["lang"] = "తెలుగు"
                st.query_params["redirect"] = "pages/telugu.py"
                st.switch_page("pages/login.py")
            st.info("మోసం గుర్తింపు ఫీచర్లను యాక్సెస్ చేయడానికి దయచేసి లాగిన్ అవ్వండి")
        else:
            st.success(f"✅  లాగిన్ అయ్యారు: **{st.session_state.username}**")
            if st.button("🚪 లాగౌట్", use_container_width=True):
                st.session_state.logged_in = False
                st.session_state.username = ''
                st.rerun()
        st.markdown("---")
        ...
        # Set default selection to current language (Telugu)
        default_index = list(languages.keys()).index('తెలుగు')
        selected_lang = st.selectbox("Select Language", list(languages.keys()), index=default_index)
        if selected_lang != "తెలుగు":
    ...
    <div class="title-wrapper">
        ...
        <div class="lang-letter lang2">అ</div>
        ...
        <div class="title-center">అభయం</div>
    </div>
    ...
    with tabs[0]:
        ...
        with col_input:
            ...
            st.markdown("### 📥 ఇన్పుట్", unsafe_allow_html=True)
            
            method = st.radio("ఇన్పుట్ పద్ధతిని ఎంచుకోండి", ["వచనం", "చిత్రం", "ఇమెయిల్"], horizontal=True)
            text_input = ""
            sender_id = ""
            
            if method == "వచనం":
                text_input = st.text_area(TEXTS['enter_sms'], height=150, placeholder=TEXTS['sms_placeholder'])
                sender_id = st.text_input(TEXTS['sender_id'], placeholder="ఉదా., BANKALERT")
            elif method == "చిత్రం":
                st.markdown('<div style="border:2px dashed #18B7BE; border-radius:20px; padding:20px;">', unsafe_allow_html=True)
                uploaded = st.file_uploader("చిత్రాన్ని అప్‌లోడ్ చేయండి", type=['png','jpg','jpeg'], label_visibility="collapsed")
                st.markdown('</div>', unsafe_allow_html=True)
                if uploaded:
                    with st.spinner("వచనాన్ని సంగ్రహిస్తోంది..."):
                        progress_bar = st.progress(0)
                        for i in range(100):
                            time.sleep(0.01)
                            progress_bar.progress(i + 1)
                        text_input = extract_text_from_image(uploaded)
                        st.text_area("సంగ్రహించిన వచనం", text_input, height=150)
            else:  # ఇమెయిల్
                text_input = st.text_area("ఇమెయిల్ యొక్క వచనం", height=150)
                sender_id = st.text_input("పంపినవారి ఇమెయిల్", placeholder="pampinavadu@example.com")
            
            col_btn1, col_btn2 = st.columns(2)
            with col_btn1:
                if st.button("🔍 భాషను గుర్తించండి", use_container_width=True):
                    if text_input:
                        with st.spinner("భాషను గుర్తిస్తోంది..."):
                            progress_bar = st.progress(0)
                            for i in range(100):
                                time.sleep(0.01)
                                progress_bar.progress(i + 1)
            ...
            with col_btn2:
                if st.button("🚨 మోసాన్ని తనిఖీ చేయండి", type="primary", use_container_width=True):
                    if not text_input:
                        st.warning("దయచేసి కొంత వచనాన్ని నమోదు చేయండి.")
                    else:
        ...
        with col_output:
            if st.session_state.results:
                ...
                if companies:
                    st.markdown("### 🏢 కంపెనీ ధృవీకరణ")
                    for comp in companies:
                        st.markdown(f"**{comp['name']}**")
                        st.caption(f"అధికారిక డొమైన్: {comp['domain']}")
                        if res['sender']:
                            if verify_sender(comp['domain'], res['sender']):
                                st.success("✅ పంపినవారి డొమైన్ సరిపోలింది")
                            else:
                                st.warning("⚠️ పంపినవారి డొమైన్ సరిపోలలేదు")
                ...
                if st.session_state.username:
                    st.markdown(f"### {TEXTS['share_alert']} / అభిప్రాయం")
                    col_f1, col_f2, col_f3 = st.columns(3)
                    with col_f1:
                        if st.button("👍 అవును"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], True, "", "")
                            st.success("ధన్యవాదాలు!")
                    with col_f2:
                        if st.button("👎 కాదు"):
                            st.session_state.show_feedback = True
                    with col_f3:
                        if st.button("🔗 ఆధార లింక్"):
                            st.session_state.show_proof = True
                    
                    if st.session_state.show_feedback:
                        comment = st.text_area("ఏమి తప్పు?")
                        if st.button("అభిప్రాయాన్ని సమర్పించండి"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, comment, "")
                            st.success("అభిప్రాయం నమోదు చేయబడింది")
                            st.session_state.show_feedback = False
                    
                    if st.session_state.show_proof:
                        proof = st.text_input("అధికారిక లింక్‌ను పేస్ట్ చేయండి")
                        if st.button("ఆధారాన్ని సమర్పించండి"):
                            save_feedback(res['text'], res['ml_prob'], res['rule_score'], result['final_score'], False, "", proof)
                            st.success("ఆధారం సమర్పించబడింది")
                            st.session_state.show_proof = False
                else:
                    st.info("అభిప్రాయం ఇవ్వడానికి దయచేసి లాగిన్ అవ్వండి.")
                
                share_text = f"నేను ఈ సందేశాన్ని అభయం‌తో తనిఖీ చేశాను. ప్రమాదం: {result['final_score']}% ({risk_level}). మీరూ తనిఖీ చేయండి!"
                col_s1, col_s2, col_s3 = st.columns(3)
                with col_s1:
                    st.markdown(f"[![WhatsApp](https://img.icons8.com/color/48/000000/whatsapp--v1.png)]({get_whatsapp_link(share_text)})")
                with col_s2:
                    st.markdown(f"[![Email](https://img.icons8.com/color/48/000000/email.png)]({get_email_link('మోసం హెచ్చరిక', share_text)})")
                with col_s3:
    ...
    with tabs[1]:
        ...
        st.write("మోసాల గురించి విద్యా సామగ్రిని ఇక్కడ చేర్చవచ్చు – కథనాలు, వీడియోలు మొదలైనవి.")
    ...
    with tabs[3]:
        ...
        with col_filter1:
            date_range = st.date_input("తేదీ పరిధిని ఎంచుకోండి", [dates[0], dates[-1]])
        with col_filter2:
            fraud_types = st.multiselect("మోసం రకం", df['Type'].unique(), default=df['Type'].unique())

        risk_threshold = st.slider("ప్రమాద పరిమితి (%)", 0, 100, 50)

        filtered_df = df[(df['Date'] >= pd.Timestamp(date_range[0])) & 
                         (df['Date'] <= pd.Timestamp(date_range[1])) &
                         (df['Type'].isin(fraud_types))]

        fig = px.line(filtered_df, x='Date', y='Fraud Cases', title='రోజువారీ మోసం కేసులు',
        ...
        fig2 = px.bar(type_counts, x='Type', y='Fraud Cases', title='రకం వారీగా మొత్తం మోసం',
    ...
    with tabs[4]:
            ...
            for _, row in posts.sort_values('timestamp', ascending=False).iterrows():
                ...
                <div style="background:rgba(10,12,16,0.6); border-radius:24px; padding:1.5rem; margin-bottom:1rem; border:1px solid #2D3A46;">
                    ...
                    <div style="margin-top:0.8rem;">
                        <span style="color:#18B7BE;">వర్గం: {row['category']}</span>
                        {f' <a href="{row["link"]}" target="_blank" style="color:#18B7BE;">🔗 లింక్</a>' if row['link'] else ''}
                    </div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("ఇంకా పోస్ట్‌లు లేవు. మొదటి పోస్ట్‌ను సృష్టించండి!")
        ...
        if st.session_state.username:
            ...
            with st.form("new_post_form"):
                ...
                category = st.selectbox(TEXTS['category'], ["సాధారణ", "మోసం హెచ్చరిక", "ప్రశ్న", "విజయ గాథ"])
                ...
                if submitted and title and content:
                    ...
                    st.success("పోస్ట్ చేర్చబడింది!")
                    st.rerun()
        else:
            st.info("పోస్ట్ చేయడానికి దయచేసి లాగిన్ అవ్వండి.")
    ...
    with tabs[5]:
        ...
        <div style="background:rgba(10,12,16,0.6); border-radius:32px; padding:2rem; border:1px solid #2D3A46;">
            <h3 style="color:#FEDD89;">అభయం – నిర్భయ మోసపు కవచం</h3>
            <p style="color:#EFF7F6;"><strong>అభయం ఎందుకు?</strong> లక్షలాది మంది ప్రజలు తమ మాతృభాషలో మోసాలకు గురయ్యే దేశంలో, మేము మిమ్మల్ని అర్థం చేసుకునే ఒక వ్యవస్థను నిర్మించాము – మీరు హిందీ, మరాఠీ, తమిళం లేదా భారతదేశంలోని 12 అధికారిక భాషలలో ఏది మాట్లాడినా. అధునాతన యంత్ర అభ్యాసం మరియు శక్తివంతమైన నియమ ఇంజిన్‌ను కలపడం ద్వారా, అభయం 1.4 బిలియన్ భారతీయులను మోసం నుండి రక్షిస్తుంది, ఒక సందేశంలో.</p>
            <hr style="border-color:#2D3A46;">
            <p><strong style="color:#18B7BE;">👩‍💻 సృష్టికర్త:</strong> <span style="color:#9BB8C9;">సాక్షి శ్రీకృష్ణ మగం</span></p>
            <p><strong style="color:#18B7BE;">🎓 చదువుతున్నది:</strong> <span style="color:#9BB8C9;">MSc డేటా సైన్స్ మరియు ఆర్టిఫిషియల్ ఇంటెలిజెన్స్</span></p>
            <p><strong style="color:#18B7BE;">🏛️ సంస్థ:</strong> <span style="color:#9BB8C9;">రామ్‌నిరంజన్ ఝుంఝున్‌వాలా కళాశాల</span></p>
            <p><strong style="color:#18B7BE;">🔧 సాంకేతికతలు:</strong> <span style="color:#9BB8C9;">Python, Scikit‑learn, XGBoost, Streamlit, కస్టమ్ రూల్ ఇంజిన్, బహుభాషా NLP, OCR, మరియు మరిన్ని.</span></p>
            <p><strong style="color:#18B7BE;">📧 సంప్రదించండి:</strong> <span style="color:#9BB8C9;">sakshimagham@gmail.com</span></p>
            <p><strong style="color:#18B7BE;">🌟 దృష్టి:</strong> <span style="color:#9BB8C9;">ప్రతి పౌరుడికి నిజ-సమయ, భాష-స్వతంత్ర రక్షణను అందించడం ద్వారా మోసం లేని డిజిటల్ భారతదేశాన్ని సృష్టించడం.</span></p>
            <p><em style="color:#DD8EA4;">ఈ ప్రాజెక్ట్ MSc డేటా సైన్స్ & AI కోర్సు యొక్క చివరి సంవత్సరం అవసరంగా సమర్పించబడింది. ఇది ఒక మార్పును తీసుకువస్తుందని మేము ఆశిస్తున్నాము.</em></p>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.error(f"**ఒక దోషం సంభవించింది:** {e}")
